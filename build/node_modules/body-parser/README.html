...
where <code>buf</code> is a <code>Buffer</code> of the raw request body and <code>encoding</code> is the
encoding of the request. The parsing can be aborted by throwing an error.</p>
<h2>Errors</h2><p>The middlewares provided by this module create errors using the
...
<h3>entity parse failed</h3><p>This error will occur when the request contained an entity that could not be
...
<h3>entity verify failed</h3><p>This error will occur when the request contained an entity that could not be
...
<h3>stream is not readable</h3><p>This error will occur when the request is no longer readable when this middleware
